...
export default function EditProperty({ property }: { property: Property }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        weekend_price:        property.weekend_price ? String(property.weekend_price) : '',
        cleaning_fee:         property.cleaning_fee ? String(property.cleaning_fee) : '0',
        security_deposit:     property.security_deposit ? String(property.security_deposit) : '0',
        currency:             property.currency,
        bedrooms:             String(property.bedrooms),
        bathrooms:            String(property.bathrooms),
        max_guests:           String(property.max_guests),
        area_sqm:             property.area_sqm ? String(property.area_sqm) : '',
        min_stay_nights:      property.min_stay_nights ? String(property.min_stay_nights) : '1',
        max_stay_nights:      property.max_stay_nights ? String(property.max_stay_nights) : '',
        amenities:            property.amenities ?? [],
        check_in_time:        (property.check_in_time ?? '14:00').substring(0, 5),
        check_out_time:       (property.check_out_time ?? '11:00').substring(0, 5),
    ...
    const deleteExistingImage = (imageId: number) => {
        ...
        router.delete(`/owner/properties/${property.id}/images/${imageId}`, {
            preserveScroll: true,
    ...
    const setAsPrimary = (imageId: number) => {
        router.post(`/owner/properties/${property.id}/images/${imageId}/primary`, {}, {
            preserveScroll: true,
            onSuccess: () => setExistingImages((prev) =>
    ...
    const submit = () => {
        post(`/owner/properties/${property.id}`, { forceFormData: true });
    };
            ...
            <div className="bg-navy-950 pt-40 pb-12">
                <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center gap-2 text-white/50 text-sm mb-4">
                        ...
                        <span>/</span>
                        <span className="text-white/80">Edit Listing</span>
                    </div>
                    <h1 className="font-display text-2xl sm:text-3xl font-semibold text-white">Edit Property</h1>
                </div>
            </div>
                                ...
                                <div className="space-y-5">
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-3 gap-3">
                                            {[
                                                { value: 'draft',    label: 'Draft',    desc: 'Not visible to guests' },
                                                { value: 'active',   label: 'Active',   desc: 'Visible and bookable' },
                                                { value: 'inactive', label: 'Inactive', desc: 'Hidden from search' },
                                            ].map((s) => (
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-navy-400 mt-1">{data.description.length} characters</p>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        {step === 2 && (
                            <div>
                                ...
                                <div className="mt-5 p-4 bg-blue-50 rounded-xl flex items-start gap-3">
                                    ...
                                    <p className="text-xs text-blue-700">Pricing tip: Research similar properties in your area to set competitive rates.</p>
                                </div>
                            </div>
                        )}
